...
class Model:
    def __init__(self, model_path):
        ...
        if self.model_type == 'sklearn' and not hasattr(self.model, 'predict'):
            raise ValueError("Loaded model does not support predict. Ensure the correct model file is provided.")
    ...
    def predict_from_data(self, data, numerical_features=None):
        ...
        if self.model_type == 'sklearn':
            ...
            raw_prediction = self.model.predict(data)
            prediction = np.argmax(raw_prediction, axis=1)  # Jika output berbentuk probabilitas
            return ["male" if p == 0 else "female" for p in prediction]
            ...
            # Konversi data ke tipe yang sesuai dengan model
            data = np.array(data, dtype=input_details[0]['dtype'])
            
            # Tambahkan batch dimension jika data adalah 1D
            if data.ndim == 1:
                data = np.expand_dims(data, axis=0)
            elif data.shape[1:] != tuple(input_details[0]['shape'][1:]):
                raise ValueError(f"Data shape {data.shape} does not match model input shape {input_details[0]['shape']}.")

            # Set tensor input untuk model
            ...
            raw_prediction = self.model.get_tensor(output_details[0]['index'])
            
            # Argmax jika output adalah probabilitas multi-class
            prediction = np.argmax(raw_prediction, axis=1)
            
            # Ubah hasil prediksi menjadi label (contoh: "male" atau "female")
            return ["male" if p == 0 else "female" for p in prediction]
